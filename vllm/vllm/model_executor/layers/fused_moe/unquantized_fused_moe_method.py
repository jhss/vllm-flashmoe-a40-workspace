...
class UnquantizedFusedMoEMethod(FusedMoEMethodBase, CustomOp):
    ...
    def is_monolithic(self) -> bool:
        ...
        if self.unquantized_backend in (
            ...
            UnquantizedMoeBackend.FLASHMOE,
        ):
            return True
        return super().is_monolithic

    @property
    def uses_flashmoe_runner_path(self) -> bool:
        return self.unquantized_backend == UnquantizedMoeBackend.FLASHMOE
    ...
    def apply_flashmoe(
        ...
        assert self.uses_flashmoe_runner_path
        assert self.moe_kernel is not None
        fused_experts = self.moe_kernel.fused_experts
        apply_with_gate_weight = getattr(fused_experts, "apply_with_gate_weight")
        return apply_with_gate_weight(
            hidden_states=x,
            w1=layer.w13_weight,
            w2=layer.w2_weight,
