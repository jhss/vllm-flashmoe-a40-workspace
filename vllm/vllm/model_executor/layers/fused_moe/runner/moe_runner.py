...
class MoERunner(MoERunnerInterface):
    ...
    @property
    def _uses_flashmoe_runner_path(self) -> bool:
        return getattr(self._quant_method, "uses_flashmoe_runner_path", False)
    ...
    def _apply_quant_method(
        ...
        """
        if self._uses_flashmoe_runner_path:
            if self._shared_experts is not None:
                raise RuntimeError(
                    "The experimental FlashMoE backend does not support shared "
                    "experts yet."
                )
            if self.gate is None or not hasattr(self.gate, "weight"):
                raise RuntimeError(
                    "The experimental FlashMoE backend currently requires an "
                    "internal gate module with a weight tensor."
                )
            if self._fse_fuse_gate:
                raise RuntimeError(
                    "The experimental FlashMoE backend does not support fused "
                    "shared-expert gates yet."
                )
            fused_out = self.routed_experts.forward_flashmoe(
                x=hidden_states,
    ...
    def _forward_impl(
        ...
        if self.gate is not None and not self._uses_flashmoe_runner_path:
            if self._fse_fuse_gate:
