...
def backend_to_kernel_cls(
    ...
    elif backend == UnquantizedMoeBackend.FLASHMOE:
        from vllm.model_executor.layers.fused_moe.experts.flashmoe import (
            FlashMoEExperts,
        )

        return FlashMoEExperts
...
def map_unquantized_backend(runner_backend: MoEBackend) -> UnquantizedMoeBackend:
    ...
    mapping = {
        ...
        "flashmoe": UnquantizedMoeBackend.FLASHMOE,
...
def make_unquantized_moe_kernel(
    ...
    if backend == UnquantizedMoeBackend.FLASHMOE:
        # FlashMoE owns distributed dispatch/combine internally. Using vLLM's
        # EP prepare/finalize here would double-dispatch tokens before the
        # FlashMoE persistent kernel sees them.
        from vllm.model_executor.layers.fused_moe.prepare_finalize.no_dp_ep import (  # noqa: E501
