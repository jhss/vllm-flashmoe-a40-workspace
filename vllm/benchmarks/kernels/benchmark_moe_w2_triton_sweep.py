...
"""W2-only Triton MoE kernel sweep for A100/SM80-style shapes."""
...
if "--cache-dir" in sys.argv:
    ...
    os.environ.setdefault(
        "TRITON_CACHE_DIR",
        tempfile.mkdtemp(prefix="vllm_moe_w2_triton_cache_"),
    )
...
def candidate_grid(base_config: dict[str, int]) -> list[Candidate]:
    ...
    # Preserve order but remove duplicates with the base config.
...
def make_inputs(args: argparse.Namespace):
    ...
    qintermediate_cache2 = torch.randn(
    ...
    return qintermediate_cache2, w2, topk_weights, topk_ids
...
def launch_w2(
    args: argparse.Namespace,
    config: dict[str, int],
    qintermediate_cache2: torch.Tensor,
    w2: torch.Tensor,
    out: torch.Tensor,
...
def main() -> None:
    ...
    qintermediate_cache2, w2, topk_weights, topk_ids = make_inputs(args)
    w1_shape = (args.num_experts, 2 * args.intermediate_size, args.hidden_size)
