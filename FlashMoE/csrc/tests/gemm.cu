...
auto reference(void *const&a, void *const&b,
  ...
  (tC = matx::apply(Converter<ElementC, AccumType>{}, matx::apply(Activation{}, (tC_interim + tBias_interim)))).
      run(exec);
  exec.sync();
...
void driver(const int &M, const int &N, const int &K, const float &rtol, const float &atol, matx::cudaExecutor &exec) {
  ...
  const auto [k_ms, e_p, r_ms] = gk_run<TileGEMM, Act, threads, sharedSize, ActM>(
    a, b, c, c_ref, bias, c_interim, bias_interim,
    ...
    switch (i) {
      ...
      default: {
        ...
        if (i >= 128 && i <= 2048) {
          ...
          driver<128, cute::max(64, 64 * (4 / sizeof(Element))), bK, pS, MMA_C, Element, ElementC>(
            i, i, i, rtol, atol, exec);
        }
      }
    }
  }
